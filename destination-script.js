...
function renderDestination(destination) {
    ...
    if (countryEl) countryEl.textContent = destination.country || "";
    if (ratingEl) ratingEl.textContent = Number(destination.rating || 0).toFixed(1);
    if (priceEl) priceEl.textContent = `$${formatPrice(destination.price_starting)}`;
    ...
    if (locationEl) locationEl.textContent = `${destination.name}, ${destination.country}`;
    if (categoryEl) categoryEl.textContent = destination.category || "—";
    if (continentEl) continentEl.textContent = destination.continent || "—";
