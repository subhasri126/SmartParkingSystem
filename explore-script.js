...
const fetchDestinations = async (params = {}) => {
    ...
    try {
        ...
        const url = queryParams.toString()
            ? `http://localhost:5000/api/destinations?${queryParams.toString()}`
...
function renderDestinations() {
    ...
    destinations.forEach(destination => {
        ...
        const continent = (destination.continent || 'other').toLowerCase();
        const category = (destination.category || 'other').toLowerCase();
        const budget = getBudgetCategory(destination.price_starting);

        card.setAttribute('data-region', mapContinentToRegion(continent));
        ...
        card.innerHTML = `
            <div class="destination-image" style="background-image: url('${destination.image_url}');">
                ${badgeHTML}
                <img src="${destination.image_url}" alt="${destination.name}" style="display: none;" onerror="this.parentElement.style.backgroundImage = 'url(https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=1200&q=80)';">
            </div>
            <div class="destination-content">
                <div class="destination-header">
                    <h3 class="destination-name">${destination.name}, ${destination.country}</h3>
                ...
                <div class="destination-footer">
                    <div class="destination-price">
                        ...
                        <span class="price-value">$${formatPrice(destination.price_starting || 0)}</span>
                    </div>
...
async function filterDestinationsWithParams(destinationFilter, budgetFilter, typeFilter, searchTerm = '') {
    const selectedRegion = destinationFilter ? destinationFilter.value.toLowerCase() : '';
    ...
    const categoryMap = {
        'city': 'City', 'beach': 'Beach', 'cultural': 'Cultural',
        'adventure': 'Mountain', 'mountain': 'Mountain',
        'nature': 'Nature', 'island': 'Island', 'desert': 'Desert'
    };
    const continentMap = {
    ...
    if (selectedType && categoryMap[selectedType]) {
        params.category = categoryMap[selectedType];
    }

    if (selectedRegion && continentMap[selectedRegion]) {
        params.continent = continentMap[selectedRegion];
    }
...
// Helpers
function mapContinentToRegion(continent) {
    const map = {
        'europe': 'europe', 'asia': 'asia', 'north america': 'americas',
        'south america': 'americas', 'africa': 'africa', 'oceania': 'oceania'
    };
    return map[continent.toLowerCase()] || 'other';
}
